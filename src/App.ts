...
import { newCalendarEventSchema } from './schemas/calendarEvent'
...
async function startServer(servicePort: number) {
  ...
  const authorizeRequest: express.Handler = (req, res, next) => {
  ...
  app.post('/api/events', authorizeRequest, async (req, res) => {
    const parsed = newCalendarEventSchema.safeParse(req.body)
    ...
    try {
      const field = await calendarEventService.createCalendarEvent(parsed.data)
  ...
  app.get('/api/users/:id/events', authorizeRequest, async (req, res) => {
    try {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const registrations =
          await calendarEventService.getRegistrationsForCalendarEventId(
            Number(req.params.id),
          )
  ...
  app.get('/api/events/:id/fields', authorizeRequest, async (req, res) => {
    try {
      const fields =
        await calendarEventService.getCustomFieldsForCalendarEventId(
          Number(req.params.id),
        )
